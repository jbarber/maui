...
*/
    ...
    else if (!strcmp(AP->name, ATTR_NODE_gpus))
      {
    	/* set the number of gpus */
    	N->CRes.Gpus = strtol(AP->value, NULL, 10);
...
int MPBSNodeUpdate(
  ...
  for (AP = PNode->attribs;AP != NULL;AP = AP->next)
    ...
    else if (!strcmp(AP->name, "gpus"))
      {
      N->CRes.Gpus = (int)strtol(AP->value, NULL,10);
      N->ARes.Gpus = MIN(N->ARes.Gpus, N->CRes.Gpus);
      }
    else if (!strcmp(AP->name,"resources_available"))
      {
      if (!strcmp(AP->resource,"mem"))
